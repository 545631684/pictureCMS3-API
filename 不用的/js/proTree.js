(function($) {
				...
				arr.map(function(item) {
					...
					if(item.child && item.child.length > 0) {
						lihtml += "<i ischek='true' class='" + self.mouIconOpen + "'></i>" +
							"<span ischek='false' id='" + item.id + "'>" + item.name + "</span>"
			...
			Tree.prototype.bindEvent = function() {
				...
				this.element.find(".menuUl li span").click(function() {
					// console.log($(this).prev().attr("class").indexOf("fa-folder-o"))
					if ($(this).prev().attr("class").indexOf("fa-folder-o") > -1) {
						var ischek = $(this).attr("ischek");
						if(ischek == 'true') {
							var menuUl = $(this).closest("li").children(".menuUl");
							$(this).prev().removeClass(self.mouIconOpen).addClass(self.mouIconClose)
							menuUl.hide();
							$(this).attr("ischek", 'false');
						} else if(ischek == 'false') {
							var menuUl = $(this).closest("li").children(".menuUl");
							menuUl.show();
							$(this).prev().removeClass(self.mouIconClose).addClass(self.mouIconOpen)
							$(this).attr("ischek", 'true')
						}
					}
